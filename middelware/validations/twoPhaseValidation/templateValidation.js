    ...
    for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
      ...
      for (let field in currentLeadData) {
        ...
        const currentFieldErrors = [];
          // console.log(value,camelField,'field')
        ...
        for (const rule of profile.valueRules) {
          ...
          if (fieldNames.includes(field)) {
            if (mode == "inclusion" || mode == "exclusion") {
              ...
              //   validateValue(valueToValidate, values, mode)
              // }
      ...
      // delete lead['index'];
       const { index = null, email, ...otherFields } = lead; // index defaults to null if missing
        try {
          let newLead = await prisma.lead.create({
            data: {
              campaignId,
              uploadId,
              email,
