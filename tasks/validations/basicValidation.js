...
// ---------------- MAIN VALIDATION ----------------
const basicValidation = async (data, profile, existingLeads) => {
  try {
    const errors = [];
    const validData = [];
    ...
    for (let index = 0; index < data.length; index++) {
      ...
      if (hasValidationErrors) {
        ...
        if (domainToIncrement != "") {
          companyEmailDomainCounter[domainToIncrement] = (companyEmailDomainCounter[domainToIncrement] ?? 0) + 1;
        }
        domainToIncrement = "";
        validData.push({
          index,
